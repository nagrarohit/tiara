        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && (
              <p>
                Sorry The cufflinkss are Out of Stock. Stay Tuned They are on
                the way!
              </p>
            )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "cufflinks" });
  let cufflinks = {};
  for (let item of products) {
    if (item.title in cufflinks) {
      if (
        !cufflinks[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        cufflinks[item.title].color.push(item.color);
      }
      if (
        !cufflinks[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        cufflinks[item.title].size.push(item.size);
      }
    } else {
      cufflinks[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        cufflinks[item.title].color = [item.color];
        cufflinks[item.title].size = [item.size];
      } else {
        cufflinks[item.title].color = [];
        cufflinks[item.title].size = [];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(cufflinks)) },
  };
}
