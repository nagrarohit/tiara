        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && (
              <p>
                Sorry The glassess are Out of Stock. Stay Tuned They are on the
                way!
              </p>
            )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "glasses" });
  let glasses = {};
  for (let item of products) {
    if (item.title in glasses) {
      if (
        !glasses[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        glasses[item.title].color.push(item.color);
      }
      if (
        !glasses[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        glasses[item.title].size.push(item.size);
      }
    } else {
      glasses[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        glasses[item.title].color = [item.color];
        glasses[item.title].size = [item.size];
      } else {
        glasses[item.title].color = [];
        glasses[item.title].size = [];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(glasses)) },
  };
}
