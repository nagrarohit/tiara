...
const hariclips = ({ products }) => {
  return (
    <div>
      <Head>
        <title>hariclips</title>
      </Head>
        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && (
              <p>
                Sorry The hariclips are Out of Stock. Stay Tuned They are on the
                way!
              </p>
            )}
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "hariclips" });
  let hariclips = {};
  for (let item of products) {
    if (item.title in hariclips) {
      if (
        !hariclips[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        hariclips[item.title].color.push(item.color);
      }
      if (
        !hariclips[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        hariclips[item.title].size.push(item.size);
      }
    } else {
      hariclips[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        hariclips[item.title].color = [item.color];
        hariclips[item.title].size = [item.size];
      } else {
        hariclips[item.title].color = [];
        hariclips[item.title].size = [];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(hariclips)) },
  };
}
export default hariclips;
